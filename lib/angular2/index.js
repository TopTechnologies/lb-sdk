  ...
  **/
  ...
  function buildModelImports(model) {
    ...
    Object.keys(model.properties).forEach((property) => {
      ...
      var hasGeoPointType = geoPointType.toLowerCase().indexOf('geopoint') !== -1;
      if(hasGeoPointType) {
          output.push('  GeoPoint');
      }
    });
    if(output.length > 0) {
        var imports = lo.uniq(output).join(',\n');
        output = [
          'import {',
          imports,
          '} from \'../index\';\n'
  ...
  function buildServiceImports(model, loadAccessToken, isIo) {
    ...
    let imports = [
      ...
      { module: 'BaseLoopBackApi, TCustomHeaders, TCustomOptions', from: '../core/base.service'},
  ...
  function buildMethodParams(model, methodName, params, isIo) {
    ...
    if (!isIo) {
        output.push(`customHeaders?: TCustomHeaders`);
        output.push(`customOptions?: TCustomOptions`);
    }
    return output.join(', ');
  }
  /**
  ...
  function paramIsContext(param) {
    return (typeof param.http !== 'undefined' && typeof param.http.source !== 'undefined' && (param.http.source == 'context' || param.http.source == 'req' || param.http.source == 'res'));
  }
  /**
   * @method paramIsBody
   * @description
